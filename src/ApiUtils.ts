...
export const applyApiSpec = <EndpointSpec extends GenericEndpointSpec = GenericEndpointSpec>(
  ...
  r: {
    http: SimpleHttpRequestHandlerBasicInterface;
    log: SimpleLoggerInterface;
  },
  ...
  for (const name in apiSpec) {
    ...
    r.http[endpointSpec.method](endpointSpec.endpoint, async (req, res, next) => {
      try {
        ...
        // 5. Authorize the request according to the authorization specification we've passed in or
        ...
        // 6. Run the primary handler for the request, returning a standardized response
